...
scplot(case) %>%
  ...
  add_text("Strength of the\nintervention effect (level)",x = n2+1, y = 11, hjust = 0, face = 3) %>%
  ...
  add_arrow(x0 = 1, y0 = -6, x1 = n2*2, y1 = -4, ends = "last", color = "grey90") %>%
  add_arrow(x0 = 1, y0 = -6, x1 = n2*2, y1 = -9, ends = "last", color = "grey90") %>%
  add_arrow(x0 = 1, y0 = -6, x1 = n2*2, y1 = -6, ends = "last") %>%
  add_text("Trend effect",x = 3, y = -3, hjust = 0, face = 3) %>%
  add_arrow(x0 = n2+1, y0 = -1, x1 = n2*2, y1 = 0, ends = "last", color = "grey90") %>%
  add_arrow(x0 = n2+1, y0 = -1, x1 = n2*2, y1 = -2, ends = "last", color = "grey90") %>%
  add_arrow(x0 = n2+1, y0 = -1, x1 = n2*2, y1 = -1, ends = "last") %>%
  add_text("Strength of the\nintervention effect (slope)",x = n2+1, y = 2, hjust = 0, face = 3,) %>%
...
base_design <- design_template(
  ...
  start_value = 15,
  level = -(15 *0.50),
  trend = rnorm(n_sims, 0, 0.1 * 15 /30),
  slope = rnorm(n_sims, 0, 0.1 * 15 /20),
